...
const { getLastSet, getLast, update: updateStock, updateSet } = require('../services/Stocks')
const { dateToIsoFormatWithTimezoneOffset, missingNumber, delInArray } = require('../scripts/utils/helper')
...
const create = async (req, res) => {
    ...
    try {
        ...
        const { rows: currenyRows, rowCount: currencyRowCount } = await currency(client, req.body.currency_code)
        let currency_id

        if (!currencyRowCount) {
            const { rows: insertCurrencyRows } = await insertCurrency(client, req.body.currency_code)
        ...
        const { rows } = await insert(client, {
            ...
            currency_id,
            ...req.body,
            status: usertype === 'admin' || usertype === 'stock_manager' ? 0 : usertype === 'boss' ? 1 : usertype.includes('domestic') ? 2 : 3,
            approver_id: usertype === 'admin' || usertype === 'stock_manager' ? userid : null
        })
        ...
        const order = {
            ...
            approver: usertype === 'admin' || usertype === 'stock_manager' ? username : null
        }
...
const putSet = async (req, res) => {
    ...
    try {
        ...
        if (!stockDiffs || !Array.isArray(stockDiffs)) {
            throw new Error('Required data missing: stockDiffs or stockDiffs is not an array');
        }

      
        const attributesArray = attributes;

        console.log("Attributes Array in the putSet function before we sent it to getLastSet function ", attributesArray);
...
sets.forEach((set, setIndex) => {
    set.products.forEach(async (product, productIndex) => { // Add async here
        ...
        if (typeof stockDiffValue !== 'number') {
            throw new Error(`Missing or invalid stockDiff for product at index ${productIndex} in set with set_id ${set.set_id}`);
        }
        ...
        const lastStockEntry = lastStocks.find(stock => stock.attributes === productAttributesString);

        if (!lastStockEntry) {
            throw new Error(`No stock entry found for product with attributes ${productAttributesString}`);
        }
...
const put = async (req, res) => {
    ...
    try {
        await client.query('BEGIN')
        console.log('attributes in the put function before send it to getLast function',req.body.product_attributes);
        const { rows: lastStock } = await getLast(req.body.product_attributes, client)
        if (lastStock.length === 0) throw 'No stock found for the product'

        const { rows, rowCount } = await updateOrderStatus({ order_id: req.params.id, ...req.body }, client)
...
const patch = async (req, res) => {
    ...
    try {
        ...
        const { rows } = await update(client, { order_id: req.params.id, currency_id, ...req.body })
...
const patchStatus = (req, res) => {
    updateStatus({ userid: req.user.userid, order_id: req.params.id, ...req.body })
        .then(({ rows }) => {
            if (rows.length === 0) return res.status(httpStatus.NOT_FOUND).send({ message: 'There is no such record.' })
...
const remove = async (req, res) => {
    ...
    try {
        ...
        const date = `${_today.getFullYear()}${String(_today.getMonth() + 1).padStart(2, '0')}${String(_today.getDate()).padStart(2, '0')}`
        ...
        if (today === dateToIsoFormatWithTimezoneOffset(new Date(`${orderDate.slice(0, 4)}-${orderDate.slice(4, 6)}-${orderDate.slice(6)}`))) {
            const { rows: orderCounterRows } = await _getOrderCounter({ suffix: orderSuffix }, client)
        ...
        console.log(e)
        client.release()
        if (e === 'There is no such record.') return res.status(httpStatus.BAD_REQUEST).send({ error: 'There is no such record.' })
        res.status(httpStatus.INTERNAL_SERVER_ERROR).send({ error: 'An error occurred.' })
    }
}
...
const createOrderNumber = async (req, res) => {
    ...
    try {
        ...
        const suffix =
            usertype === 'domestic_market_manager' || usertype === 'domestic_market_marketing'
                ? 'A'
                : usertype === 'foreign_market_manager' || usertype === 'foreign_market_marketing'
                ? 'B'
                : 'C'

        let orderCounter = 10
        const { rows: orderCounterRows } = await _getOrderCounter({ suffix }, client)
        if (orderCounterRows.length === 0) await _insertOrderCounter({ suffix, date: today, counter: [orderCounter] })
        else {
            ...
            if (dateToIsoFormatWithTimezoneOffset(new Date(_date)) === today) {
                ...
                newCounter.length > 0 && (orderCounter = miss ?? newCounter[newCounter.length - 1] + 1)
            }
        ...
        await client.query('COMMIT')
        client.release()
        res.status(httpStatus.CREATED).send({ order_number: `${date}-MG-${orderCounter.toString().padStart(3, '0')}-${suffix}` })
    } catch (e) {
        console.log(e)
        client.release()
        res.status(httpStatus.INTERNAL_SERVER_ERROR).send({ error: 'An error occurred.' })
...
const delOrderNumber = async (req, res) => {
    ...
    try {
        ...
        await client.query('COMMIT')
        client.release()
        res.status(httpStatus.OK).send({ message: 'Order Number deleted successfully.' })
    } catch (e) {
        console.log(e)
        client.release()
        res.status(httpStatus.INTERNAL_SERVER_ERROR).send({ error: 'An error occurred.' })

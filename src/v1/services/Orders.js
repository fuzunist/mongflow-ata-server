...
const updateOrderStatus = (data, client) => {
    const query = 'UPDATE orders SET products = $1, order_status = $2 WHERE order_id = $3 RETURNING order_id'
...
const updateOrderStatusSet = (data, client) => {
    ...
    const query = 'UPDATE orders SET sets = $1, order_status = $2 WHERE order_id = $3 RETURNING order_id';
...
const update = (client, data) => {
    return client.query(
        ...
        [data.customerid, data.currency_id, data.subtotal, data.total_with_tax, data.products, data.sets, data.order_status, data.order_id]
    )
}

const updateStatus = (data) => {
    return process.pool.query('UPDATE orders SET status = $1, approver_id = $2 WHERE order_id = $3 RETURNING *', [
        data.status,
        data.userid,
        data.order_id
    ])
...
const _insertOrderCounter = (data, client) => {
    const query = 'INSERT INTO ordercounter (suffix, counter, date) VALUES ($1, $2, $3) RETURNING *'
...
const _updateOrderCounter = (data, client) => {
    const query = 'UPDATE ordercounter SET counter = $1, date = $2 WHERE suffix = $3 RETURNING *'

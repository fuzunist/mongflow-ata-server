...
//   const query = `SELECT 
//   recipemateriallogs.product_id, 
//   COALESCE(jsonb_object_agg(attr.attribute_name, val.value), '{}') AS attributeDetails
// FROM 
// recipemateriallogs
// LEFT JOIN LATERAL (
//   SELECT 
...
//       value::int AS val_id
//   FROM 
//       jsonb_each_text(recipemateriallogs.attributes::jsonb)
// ) AS attr_val ON TRUE
// LEFT JOIN attribute AS attr ON attr.attribute_id = attr_val.attr_id
// LEFT JOIN value AS val ON val.value_id = attr_val.val_id
// WHERE 
// recipemateriallogs.id= $1
// GROUP BY 
//   recipemateriallogs.product_id;
// `;
...
const getStock = (data, client) => {
  const query =
    "SELECT id FROM recipematerialstocks WHERE product_id = $1";
...
const insertStock = (data, client) => {
  const query =
    "INSERT INTO recipematerialstocks (product_id, price, quantity ) VALUES( $1, $2, $3) RETURNING *";
...
const delLog = (id, client) => {
  const query = `DELETE FROM recipemateriallogs WHERE id = $1`;
  const values = [id];
...
const delStock = (id, client) => {
  const query = `DELETE FROM recipematerialstocks WHERE id = $1`;
  const values = [id];
...
const getLog = (id, client) => {
  const query =
    "SELECT product_id, attributes, price, quantity, customer_id, customer_city, customer_county FROM recipemateriallogs WHERE id = $1";
  const values = [id];

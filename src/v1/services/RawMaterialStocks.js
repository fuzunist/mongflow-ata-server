...
//   const query = `SELECT 
//   rawmateriallogs.product_id, 
//   COALESCE(jsonb_object_agg(attr.attribute_name, val.value), '{}'::jsonb) AS attributeDetails
// FROM 
//   rawmateriallogs
// LEFT JOIN LATERAL (
//   SELECT 
...
//       value::int AS val_id
//   FROM 
//       jsonb_each_text(rawmateriallogs.attributes::jsonb)
// ) AS attr_val ON TRUE
// LEFT JOIN attribute AS attr ON attr.attribute_id = attr_val.attr_id
// LEFT JOIN value AS val ON val.value_id = attr_val.val_id
// WHERE 
//   rawmateriallogs.id = $1
// GROUP BY 
//   rawmateriallogs.product_id;
